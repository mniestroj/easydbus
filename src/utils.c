...
int push_variant(lua_State *L, GVariant *value)
{
    ...
    case G_VARIANT_CLASS_HANDLE:
        lua_pushinteger(L, g_variant_get_handle(value));
        break;
...
static GVariant *to_variant(lua_State *L, int index, const char *sig);

static GVariant *to_tuple(lua_State *L, int index, const char *sig)
{
    ...
    for (i = 1; i <= n_arg; i++) {
        ...
        g_variant_builder_add_value(&elem_builder, to_variant(L, lua_gettop(L), elem_sig));
        g_free(elem_sig);
        lua_pop(L, 1);
        startptr = endptr;
    }
...
static GVariant *to_array(lua_State *L, int index, const char *sig)
{
    ...
    if (sig[1] == '{') {
        ...
        while (lua_next(L, index) != 0) {
            ...
            g_variant_builder_add_value(&elem_builder, to_variant(L, top + 1, key_sig));
            g_variant_builder_add_value(&elem_builder, to_variant(L, top + 2, val_sig));
        ...
        for (i = 1; i <= n_arr; i++) {
            lua_rawgeti(L, index, i);
            g_variant_builder_add_value(&array_builder, to_variant(L, top + 1, &sig[1]));
            lua_pop(L, 1);
        }
    }
...
static GVariant *to_variant(lua_State *L, int index, const char *sig)
{
    ...
    if (sig && sig[0] != 'v') {
        ...
        case 'o':
            ...
            break;
        case 'a':
            value = to_array(L, index, sig);
            break;
        case '(':
            value = to_tuple(L, index, sig);
            break;
        default:
        ...
        case LUA_TTABLE:
            if (ed_istype(L, index)) {
                ...
                value = to_variant(L, lua_gettop(L), lua_tostring(L, -2));
                lua_pop(L, 2);
            } else if ((n_arr = lua_rawlen(L, index)) > 0) {
                value = to_array(L, index, "ai");
            } else {
                value = to_array(L, index, "a{sv}");
            }
            break;
        default:
...
GVariant *range_to_tuple(lua_State *L, int index_begin, int index_end, const char *sig)
{
    ...
    if (sig) {
        ...
        for (i = index_begin; i < index_end; i++) {
            ...
            g_variant_builder_add_value(&builder, to_variant(L, i, subsig));
            g_free(subsig);
            startptr = endptr;
        }
    } else {
        for (i = index_begin; i < index_end; i++)
            g_variant_builder_add_value(&builder, to_variant(L, i, NULL));
    }
